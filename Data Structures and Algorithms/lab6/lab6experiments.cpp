...
int main(int argc, char* argv[]){
    if(argc!=3){
        ...
        Timer t;
        std::fstream fout;
        fout.open("timeRecords.csv", std::ios::out | std::ios::app);
        fout << "Shape" << ',' << "Elapsed Time" << ',' << "Sort Type" << std::endl;
        std::string sortType;
        switch (algchoice)
        {
        case 1:
            sortType = "Quick Sort Last";
            break;
        case 2:
            sortType = "Quick Sort Middle";
            break;
        case 3:
            sortType = "Quick Sort Median of 3";
            break;
        case 4:
            sortType = "Quick Sort Random";
            break;        
        default:
            break;
        }
        for(int i=0;i<5;i++){
            ...
            fout << dataName[i] << ',' << t.currtime() << ',' << sortType << std::endl;
