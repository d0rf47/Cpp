...
public:
	...
	BST(const BST& rhs) 
	{
		 copyTo(this->root_, rhs.root_);
	}
	void copyTo(Node*& to, const  Node* from)
	{
		if (from == nullptr)
			to == nullptr;
		else
		{
			to = new Node(from->data_);			
			copyTo(to->left_, from->left_);
			copyTo(to->right_, from->right_);
		}
	...
	{
		if (!search(data, root_))
			return;
		removeNode(root_, data);
	}
	void removeNode(Node*& subRoot, const  T& data)
	{
		Node* curr = subRoot;
		Node* parent = nullptr;
		...
		else if (curr->left_ && curr->right_)
		{
			...
			int nodeData = successor->data_;
			removeNode(subRoot, successor->data_);
			curr->data_ = nodeData;
		}
		else
		{
	...
	int getDepth(Node*& subRoot, const T& data, int cnt)
	{
		if (subRoot == nullptr)
			return 0;
		...
		int lvl = getDepth(subRoot->left_, data, cnt + 1);
		if (lvl != 0)
			return lvl;
		lvl = getDepth(subRoot->right_, data, cnt + 1);		
